...
int search(int start, int end, int num, int numbers[]);
...
int main(void)
{
    ...
    printf("%d\n", size);
    printf("Please enter your number (0 - 500): ");
    scanf("%d", &n);

    int result = search(numbers[0], numbers[size - 1], n, numbers);
...
int search(int start, int end, int num, int numbers[])
{
    ...
    int current = (int)((start + end) / 2);

    if (num == numbers[current])
        return current;
    else if (num < numbers[current])
    {
        return search(start, numbers[current - 1], num, numbers);
    }
    else if (num > numbers[current])
    {
